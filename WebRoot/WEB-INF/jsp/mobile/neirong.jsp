...
<p>上一篇：<a href="${path}/tiezi/${last.id}_1_1.html" style="color:#3399CC">${last.name}</a></p>
</c:if>
<c:if test="${not empty next}">
<p>下一篇：<a href="${path}/tiezi/${next.id}_1_1.html" style="color:#3399CC">${next.name}</a></p>
</c:if>
</div>
</div>
</div>
...
<c:if test="${!empty huifumap.list}">
     ...
     <span>当前第${huifumap.pagestart}页/总${huifumap.totalPages}页</span>
        ...
        <a href="${path}/tiezi/${map.tiezi.id}_1_${orderby}.html" >首页</a>
        <a href="${path}/tiezi/${map.tiezi.id}_${huifumap.pagestart-1}_${orderby}.html" >上一页</a>
        </c:if>
         <c:if test="${huifumap.pagestart!=huifumap.totalPages}">
          <a href="${path}/tiezi/${map.tiezi.id}_${huifumap.pagestart+1}_${orderby}.html" >下一页</a>
        <a href="${path}/tiezi/${map.tiezi.id}_${huifumap.totalPages}_${orderby}.html" >末页</a>
         </c:if>
      </div>
</c:if>
