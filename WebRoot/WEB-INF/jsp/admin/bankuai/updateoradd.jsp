...
<html xmlns="http://www.w3.org/1999/xhtml">
 <head>
    ...
    function findcheckboxalla(){
    	$("input[name='findcheckboxall']").each(function(){   
	    	if(this.checked==true){
	    		 $("input[name='findcheckbox']").each(function(){   
	                 this.checked=true;  
	             });
	    	}else{
	    		$("input[name='findcheckbox']").each(function(){   
    ...
    function yanzheng(){
    	...
    	var findgroup ="";    
    	  $('input[name="findcheckbox"]:checked').each(function(){
    		  findgroup+=$(this).val()+",";
    	  });  
    	  $("#fatiegroup").val(fatiegroup);
    	  $("#huifugroup").val(huifugroup);
    	  $("#findgroup").val(findgroup);
    	$("#form").submit();
    }
  ...
  $(function(){
	...
	var findgroup=$("#findgroup").val();
	 ...
	 $("input[name='findcheckbox']").each(function(){   
         if (findgroup.indexOf($(this).val()+',') >-1) {  
             this.checked=true;  
         }   
     });
  });
  ...
  function getpinyin(){
	  ...
	  var param={};
		 ...
		 $.ajax({
			   type: "POST",
			   url: "${path}/admin/bankuai/getpinyin.do",
			   data: param,
			   dataType: "json", 
			   async: false,
    ...
    <form method="post" class="form-x" action="${path}/admin/bankuai/updateoradd.do" id="form">
      <div class="form-group">
        ...
        <div class="field">
          ...
          <button class="button border-main icon-check-square-o" type="button" onclick="getpinyin();">获取拼音</button>
      ...
      <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input" name="yuming" value="${bankuai.yuming}" size="100" id="yuming" />
          <div class="tips">此处的拼音将是二级目录</div>
        </div>
      </div>
      <div class="form-group">
        <div class="label">
      ...
      <div class="form-group">
        ...
        <div class="field">
		  <input type="checkbox" name="findcheckboxall" onclick="findcheckboxalla();"/>全选
        <c:forEach items="${jifengouplist}" var="jifengoup">
		  <input type="checkbox" name="findcheckbox" value="f${jifengoup.id}"/>${jifengoup.name}
        </c:forEach>
        <input type="hidden" name="findgroup" id="findgroup" value="${bankuai.findgroup}"/>
