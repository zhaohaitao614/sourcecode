  ...
  <div class="body-content">
    <form method="post" class="form-x" action="${path}/admin/link/updateoradd.do" id="form">
      ...
      <div class="form-group">
        ...
        <div class="field">
	        <select class="input" name="bankuaiid">
	        	<option value="0" <c:if test="${bankuai.id==0 }">selected="selected"</c:if> >首页</option>
	        	<c:forEach items="${list}" var="bankuai">
		        	<option value="${bankuai.id}" <c:if test="${bankuai.id==link.bankuaiid }">selected="selected"</c:if> >${bankuai.name}</option>
	        	</c:forEach>
