	...
	if(confirm("确认删除此帖子吗?删除此贴将导致此贴所有回复消失")){
		var param={};
		 ...
		 $.ajax({
			   ...
			   async: false,
			   success: function(data){
				   window.location.href='${path}/${map.tiezi.zyuming}/1_0_0.html';
			   }
		 });
	}
}
...
<!-- 					<div>
               ...
               <c:if test="${not empty  listtiezi}">
               <tr>
               <td class="plc plm">
					<div class="mtw mbw">
					<h3 class="pbm mbm bbda">相关帖子</h3>
					<ul class="xl xl2 cl">
					<c:forEach items="${listtiezi}" var="tiezi">
						<li>• <a href="${path}/tiezi/${tiezi.id}_1_1.html" title="${tiezi.name }" target="_blank">${tiezi.name }</a></li>
					</c:forEach>
					</ul>
					</div>
				</td>
        ...
        <em>上一篇：<a href="${path}/tiezi/${last.id}_1_1.html" style="color:#3399CC">${last.name}</a></em>
        </c:if>
        <c:if test="${not empty next}">
        <em>下一篇：<a href="${path}/tiezi/${next.id}_1_1.html" style="color:#3399CC">${next.name}</a></em>
        </c:if>
        </div>
        </td>
        </tr>
    ...
    <c:if test="${!empty huifumap.list}">
     <div class="bm bw0 pgs cl"> 
        ...
        <a href="${path}/tiezi/${map.tiezi.id}_${huifumap.pagestart-1}_${orderby}.html" class="prev"></a>
         ...
         <span class="y pgb" ><a href="${path}/${map.tiezi.zyuming}/1_0_0.html">返回列表</a></span> 
      </div>
    </c:if>
    </div> 
